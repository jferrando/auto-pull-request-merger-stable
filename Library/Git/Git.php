...
class GitRepo {
    ...
    /**
     * Runs a git pull on the current branch
     *
     * @access  public
     * @return  string
     */
    ...
    /**
     * Remove tag from the local and remote repo
     * @param string $tag
     */
    public function removeTag($tag)
    {
        $this->run("tag -d $tag");
        $this->run("push origin :refs/tags/$tag");
    }
    ...
    {
        return $this->run("describe");
    }


    /**
     * Get the last tag applied
     * @return string
     */
    public function getLastTag()
    {
        $describeDetails = explode("-", $this->describe());
        return trim($describeDetails[0]);
    }
     ...
     * @return array
     */
    public function getCommitList($from, $to)
    {
        $output = $this->run("log --format=%B $from..$to");
        $results = explode("\n", $output);
